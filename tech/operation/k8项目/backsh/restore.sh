...
BACKUP_PATH_DATA=$BACKUP_PATH/data/restore BACKUP_PATH_LOG=$BACKUP_PATH/log
...
for file_yaml in $file_list; do
     ...
     echo "`date` Create resource ${result}." 2>&1 >> $RESTORE_LOG_FILE done;

## show stats
message="`date` Restore ${COUNT} yaml files in all."
echo ${message}
echo ${message} 2>&1 >> $RESTORE_LOG_FILE
echo "`date` Kubernetes Restore completed, all done." 2>&1 >> $RESTORE_LOG_FILE exit 0;